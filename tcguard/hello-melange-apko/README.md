...
Build an apk for all architectures using melange: (changed sdk and used ori yaml; amd64 only)
```
docker run --rm --privileged -v "${PWD}":/work  \
    --entrypoint=melange --workdir=/work \
    ghcr.io/wolfi-dev/sdk build melange.yaml.ori \
    --arch amd64 \
...
melange build melange.yaml.ori \
...
# A personal access token with the "write:packages" scope (DPD: debug flag deprecated)
GITHUB_TOKEN="*****"
